            ...
            <div className="flex flex-col">
                {experienceData.map((item) => 
                    (
                        <button 
                            key={item.id}
                            className={`w-70 h-12 border-l-2 text-start pl-5 ${experience===item.id? "w-70 h-12 border-l-2 text-start pl-5 bg-[#64ffda]/4 text-[#64ffda] border-[#64ffda]" :  "hover:bg-[#64ffda]/5 hover:text-[#64ffda] text-[#8892b0] border-[#233554]"}`}
                            onClick={() => {
                ...
                experience === item.id && (
                    ...
                    >
                        <h1 className="text-2xl font-semibold">
                            {item.title} <span className="text-[#64ffda]">{item.company}</span>
                        </h1>
                        <p className="text-md text-[#8892b0] font-mono">{item.duration}</p>
                        <ul className="list-none space-y-2 text-[#8892b0] font-mono text-sm">
                            {item.points.map((point, id) => {
                                return <li key={id}><span className="text-[#64ffda] mr-2">▹</span>{point}</li>
                            })}
                        </ul>
                    </motion.div>
                )
